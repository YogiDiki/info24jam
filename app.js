...
function setupEvents() {
    ...
    on('searchBtn', 'click', () => searchReports(document.getElementById('searchInput').value));
...
async function showReportList() {
    try {
        ...
        container.innerHTML = data?.length 
            ...
            : '<p style="text-align:center;color:#666;padding:40px">Belum ada laporan</p>';
...
async function showStatistics() {
    try {
        ...
        container.innerHTML = `
            <h3 style="font-size:16px;font-weight:700;margin-bottom:16px">Total Laporan: ${total}</h3>
...
async function installPWA() {
    if (!deferredPrompt) {
        return notify('App sudah terinstall', 'info');
    }
